...
- (void)viewDidLoad {
    ...
    if ([language isEqualToString:country]) {
        StringURL = [NSString stringWithFormat:@"%@%@",SERVER_URL, @"privacy.php?lang=ar"];
    } else {
        StringURL = [NSString stringWithFormat:@"%@%@",SERVER_URL, @"privacy.php?lang=en"];
    }
    ...
    NSURLSessionDataTask *task = [session dataTaskWithRequest:request completionHandler: ^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        if (!error) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [privacyWebView loadHTMLString:[headerString stringByAppendingString:contentOfURL] baseURL:nil];
            });
